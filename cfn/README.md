...
- `teslaDriverVersion` (where applicable): [Tesla driver version](https://docs.nvidia.com/datacenter/tesla/index.html) to install when `NVIDIA-Tesla` option is selected for `driverType` or `console-with-NVIDIA_Tesla_runfile_Driver` option is selected for `sessionType`. To find the approriate driver version, go to [NVIDIA Driver Downloads](http://www.nvidia.com/Download/index.aspx). Select correct **Operating System**, and refer to [To download a public NVIDIA driver](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/install-nvidia-driver.html#public-nvidia-driver) table for the **Product Type**, **Product Series**, and **Product** values to use for your `instanceType`
