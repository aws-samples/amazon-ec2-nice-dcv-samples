...
- `UbuntuDriverVersion` (Ubuntu only): NVIDIA [Enterprise Ready Driver (ERD)](https://ubuntu.com/server/docs/nvidia-drivers-installation) version to install when `console-with-Ubuntu_repo_Driver` is selected  under `sessionType`
...
For Ubuntu, you can install [Enterprise Ready Drivers (ERD) packages](https://ubuntu.com/server/docs/nvidia-drivers-installation) by selecting `console-with-Ubuntu_repo_Driver`. Use  `ubuntuDriverVersion` parameter value to specify driver version, e.g. 535.
