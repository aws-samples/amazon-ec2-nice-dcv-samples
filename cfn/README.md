...
- `sessionType` (Linux) : [session type](https://docs.aws.amazon.com/dcv/latest/adminguide/managing-sessions-intro.html) and [GPU driver installation](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/install-nvidia-driver.html) option
  ...
  - `console` or `console-with-*`: DCV captures content of desktop screen which is normally rendered by X server (Xorg)
  - `console-(Xdcv)*`: console session served by [Xdcv](https://docs.aws.amazon.com/dcv/latest/adminguide/xdcv-console-sessions.html) providing dynamic resizing and multi-monitor support
...
- [Install NVIDIA GPU driver, CUDA Toolkit, NVIDIA Container Toolkit on Amazon EC2 instances running RHEL/Rocky Linux/AlmaLinux 8/9/10](https://repost.aws/articles/ARpmJcNiCtST2A3hrrM_4R4A/install-nvidia-gpu-driver-cuda-toolkit-nvidia-container-toolkit-on-amazon-ec2-instances-running-rhel-rocky-linux-almalinux-8-9-10)
